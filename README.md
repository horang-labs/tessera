...
Published builds include anonymous product-interaction telemetry that you can disable during onboarding or in Settings. This includes which named controls and features are used and a coarse mobile-or-desktop classification computed locally, but never visible UI text, prompts, messages, file paths, command output, repository names, raw device details, or account details.
