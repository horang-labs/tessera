...
import { createControlAuthorityRegistry } from '@/lib/control/authority';
import { createInMemoryControlAuditHistory } from '@/lib/control/audit';
...
test('CLI manages default and explicit provider sets through Provider Integration', async () => {
  ...
  const managedGrant = authority.grant({
  ...
  const service = createControlService({
    ...
    providerSkillIntegration: integration,
  ...
  const handler = createControlHttpHandler({
    ...
    resolveManagedCredential: (credential) => (
      credential === managedCredential
        ? {
            authorityToken: managedGrant.token,
            agentEnvironment: 'wsl',
            projectId: 'managed-project',
            sessionId: 'managed-session',
          }
