...
test('late replay events do not restart unread completed sessions', () => {
  ...
  assert.match(clientMessageHandlersSource, /case 'replay_events':\s*sessionStore\.touchSessionActivity\(msg\.sessionId, getLatestReplayEventTimestamp\(msg\.events\)\);\s*if \(shouldStartTurnFromReplayEvents\(sessionStore, msg\.sessionId, msg\.events\)\)/);
});
