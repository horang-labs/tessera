...
import type { Panel, PanelNode, PanelStore, PanelStoreState, TabPanelData } from '@/types/panel';
...
export const usePanelStore = create<PanelStore>()((set, get) => ({
  ...
  assignSession: (panelId, sessionId) => {
    ...
    if (sessionId !== null) {
      ...
      for (const [tabId, td] of Object.entries(state.tabPanels)) {
        const duplicate = Object.values(td.panels).find(
          p => p.sessionId === sessionId && !(tabId === state.activeTabId && p.id === panelId)
        );
        if (duplicate) {
          if (process.env.NODE_ENV === 'development') {
            console.warn('[panel-store] assignSession() no-op: session already open in tab', tabId, 'panel', duplicate.id);
          }
          return;
        }
      }
    ...
    if (panelId === tabData.activePanelId) {
      useSessionStore.getState().setActiveSession(sessionId);
    }
  },
