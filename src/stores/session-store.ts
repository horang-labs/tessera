...
interface SessionState {
  ...
  markSessionRunning: (sessionId: string, tesseraSessionId: string) => void;
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  markSessionRunning: (sessionId, tesseraSessionId) =>
    set((state) => ({
      projects: state.projects.map((project) => ({
        ...project,
        sessions: project.sessions.map((s) =>
  ...
  updateLinkedTaskWorkflowStatus: (sessionId, workflowStatus) => {
    ...
    if (nextWorkflowStatus === 'done' && session.isRunning) {
  ...
  toggleArchive: (sessionId, archived) => {
    ...
    const prevArchived = get().getSession(sessionId)?.archived;
