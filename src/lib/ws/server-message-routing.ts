import { checkAllCliStatuses } from '@/lib/cli/connection-checker';
...
async function checkCliStatusForWebSocketUser(
  ...
  try {
    const agentEnvironment = await getAgentEnvironment(userId);
    const results = await checkAllCliStatuses();
    primeProviderStatusCache(agentEnvironment, results);
    sendToUser(userId, {
...
async function checkProviderStatusesForEnvironment(
  ...
  const results = await checkAllCliStatuses();
  return primeProviderStatusCache(agentEnvironment, results);
}

function primeProviderStatusCache(
  agentEnvironment: 'native' | 'wsl',
  results: Awaited<ReturnType<typeof checkAllCliStatuses>>,
): ProviderMeta[] {
  // Results include both native and wsl probes; only cache entries that match
  // the user's active environment so downstream consumers get the spawn view.
  const entries = results
    .filter((r) => r.environment === agentEnvironment)
    .map((r) => ({ providerId: r.providerId, status: r.status, version: r.version }));

  cliProviderRegistry.primeStatusCache(agentEnvironment, entries);
