...
export class ProcessManager {
  ...
  private registerRunningProcess(
    ...
    const processInfo = this.createProcessInfo(sessionId, userId, provider, cliProcess);
  ...
  private async spawnAndRegisterSession(
    ...
    try {
      ...
      this.registerRunningProcess(sessionId, userId, provider, cliProcess, lifecycle, cwd);
      return true;
    } catch (err) {
      logger.error({
        sessionId,
