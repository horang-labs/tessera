...
for process in /proc/[0-9]*; do
  for fd in "$process"/fd/*; do
    # Use the shell's inode comparison instead of spawning readlink once per
    # descriptor. Large agent hosts can expose enough descriptors for the
    # subprocess-per-FD scan to exceed the fail-closed probe timeout.
    if [ "$fd" -ef "$1" ] 2>/dev/null; then
      count=$((count + 1))
      break
    fi
  done
