...
type Listener = (workDir: string, stats: WorktreeDiffStats | null, userIds: string[]) => void;
...
function notifyListeners(workDir: string, stats: WorktreeDiffStats | null, userIds: string[]): void {
  for (const listener of getState().listeners) {
    try {
      listener(workDir, stats, userIds);
    } catch {
...
async function runCompute(workDir: string, userIds: string[]): Promise<WorktreeDiffStats | null> {
  ...
  const promise = (async () => {
    try {
      ...
      notifyListeners(workDir, stats, userIds);
      return stats;
    } finally {
