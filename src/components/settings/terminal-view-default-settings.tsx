'use client';

import { useId } from 'react';
import { SquareTerminal, MessageSquare } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useI18n } from '@/lib/i18n';
import { useSettingsStore } from '@/stores/settings-store';
import type { TerminalSessionDefaultView } from '@/lib/settings/types';

const OPTIONS: ReadonlyArray<{
  view: TerminalSessionDefaultView;
  icon: typeof SquareTerminal;
}> = [
  { view: 'terminal', icon: SquareTerminal },
  { view: 'chat', icon: MessageSquare },
];

/**
 * Which surface a PTY session opens on.
 *
 * Separate from the execution-mode setting above it: that one decides whether a
 * new session is a terminal or a GUI chat at all, while this only picks the
 * default view for sessions that are already terminals. Sessions the user has
 * toggled keep their own choice regardless of this.
 */
export default function TerminalViewDefaultSettings() {
  const { t } = useI18n();
  const value = useSettingsStore((state) => state.settings.terminalSessionDefaultView);
  const isSaving = useSettingsStore((state) => state.pendingSaveCount > 0);
  const updateSettings = useSettingsStore((state) => state.updateSettings);

  const groupId = useId();
  const descriptionId = `${groupId}-description`;
  const noteId = `${groupId}-note`;

  return (
    <fieldset
      className="space-y-3"
      disabled={isSaving}
      aria-describedby={`${descriptionId} ${noteId}`}
    >
      <legend className="font-medium text-(--text-primary)">
        {t('settings.terminalView.title')}
      </legend>
      <p id={descriptionId} className="-mt-2 text-xs leading-5 text-(--text-muted)">
        {t('settings.terminalView.description')}
      </p>

      <div className="grid gap-2 sm:grid-cols-2">
        {OPTIONS.map(({ view, icon: Icon }) => {
          const selected = value === view;
          const inputId = `${groupId}-${view}`;

          return (
            <label
              key={view}
              htmlFor={inputId}
              className={cn(
                'relative flex cursor-pointer gap-3 rounded-xl border px-3 py-3 text-left transition-colors',
                'has-focus-visible:outline-2 has-focus-visible:outline-offset-2 has-focus-visible:outline-(--accent)',
                selected
                  ? 'border-[color-mix(in_srgb,var(--accent)_35%,transparent)] bg-[color-mix(in_srgb,var(--accent)_10%,transparent)]'
                  : 'border-(--divider) bg-(--sidebar-bg) hover:border-(--accent)/25',
                isSaving && 'cursor-wait opacity-70',
              )}
            >
              <Icon className="mt-0.5 h-4 w-4 shrink-0 text-(--accent-hover)" aria-hidden="true" />
              <span className="min-w-0 flex-1">
                <span className="text-sm font-medium text-(--text-primary)">
                  {t(`settings.terminalView.${view}.label`)}
                </span>
                <span className="mt-1.5 block text-xs leading-5 text-(--text-muted)">
                  {t(`settings.terminalView.${view}.description`)}
                </span>
              </span>
              <span className="relative mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center">
                <input
                  type="radio"
                  id={inputId}
                  name={groupId}
                  value={view}
                  data-testid={`terminal-view-default-${view}`}
                  checked={selected}
                  onChange={() => {
                    if (selected) return;
                    void updateSettings({ terminalSessionDefaultView: view });
                  }}
                  className="h-4 w-4 accent-(--accent)"
                />
              </span>
            </label>
          );
        })}
      </div>

      <p id={noteId} className="text-xs leading-5 text-(--text-muted)">
        {t('settings.terminalView.perSessionNote')}
      </p>
    </fieldset>
  );
}
