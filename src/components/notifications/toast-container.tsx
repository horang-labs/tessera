...
// Simple action toast (success/error/warning)
function ActionToastItem({ t: toastItem, onDismiss }: { t: ActionToast; onDismiss: () => void }) {
  ...
  const Icon = toastItem.type === 'success' ? CheckCircle : toastItem.type === 'error' ? XCircle : AlertTriangle;
  const color = toastItem.type === 'success' ? 'var(--success)' : toastItem.type === 'error' ? 'var(--error)' : 'var(--warning)';
