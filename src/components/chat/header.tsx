...
import { Pencil, Check, Hash, X as XIcon, Sparkles, MoreHorizontal, Loader2, GitBranch } from 'lucide-react';
...
export function Header({ sessionId, isSinglePanel = false }: HeaderProps) {
  const { t } = useI18n();
      ...
      >
        ...
        {isEditingTitle ? (
          ...
          <div className="group flex min-w-0 flex-1 items-center gap-1.5">
            ...
            {/* Sparkle button — AI title generation */}
            <button
              onClick={() => generateTitle(sessionId)}
              disabled={isGeneratingTitle}
              className={cn(
                'p-0.5 rounded transition-all duration-150 shrink-0',
                isGeneratingTitle
                  ? 'cursor-default bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] text-(--accent) opacity-100'
                  : [
                      'text-(--text-muted) hover:text-(--accent) active:scale-90',
                      'hover:bg-[color-mix(in_srgb,var(--accent)_10%,transparent)]',
                      'opacity-0 group-hover:opacity-100',
                    ]
              )}
              data-testid="header-generate-title"
              aria-label="Generate title"
              title={isGeneratingTitle ? t('chat.generatingTitle') : t('chat.generateTitle')}
            >
              {isGeneratingTitle ? (
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {(panelCount >= 2 || panels[activePanelId]?.sessionId) && (
            <button
              onClick={() => {
                const activePanel = panels[activePanelId];
                if (activePanel?.sessionId) {
                  assignSession(activePanelId, null);
                } else {
                  closePanel(activePanelId);
                }
              }}
              title={panels[activePanelId]?.sessionId ? t('chat.closeSession') : t('panel.closePanel')}
              aria-label={panels[activePanelId]?.sessionId ? t('chat.closeSession') : t('panel.closePanel')}
