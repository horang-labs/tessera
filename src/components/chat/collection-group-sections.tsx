...
const TASK_TITLE_ACTION_MASK =
  'linear-gradient(to right, #000 0%, #000 calc(100% - 5.75rem), rgba(0,0,0,0.35) calc(100% - 4.25rem), transparent calc(100% - 2.75rem), transparent 100%)';
const TASK_TITLE_ACTION_MASK_WITH_STOP =
  'linear-gradient(to right, #000 0%, #000 calc(100% - 6.75rem), rgba(0,0,0,0.35) calc(100% - 5.25rem), transparent calc(100% - 3rem), transparent 100%)';
...
function SessionReferenceDragHint({ label }: { label: string }) {
  return (
    <span
      role="img"
      aria-label={label}
      title={label}
      className="flex h-5 w-4 cursor-grab items-center justify-center rounded text-(--text-muted) opacity-70 transition-colors duration-150 hover:bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] hover:text-(--accent)"
    >
...
function SubSessionRow({
  ...
  return (
    <div
      className={cn(
        'group/sub relative my-px flex cursor-pointer items-center gap-1.5 rounded px-2 py-1 text-[0.75rem] transition-colors duration-150',
        isSelected
...
export function TaskItemRow({
  ...
  return (
    <div className="task-item-container" data-item-id={task.id}>
      ...
      <div
        draggable={!disableDnd && !isRenaming && !isPending}
        aria-disabled={isPending || undefined}
        onDragStart={!disableDnd && !isRenaming && !isPending ? onDragStart : undefined}
        onDragEnd={!disableDnd && !isRenaming && !isPending ? onDragEnd : undefined}
        ...
        className={cn(
          ...
          !disableDnd && !isPending && 'cursor-grab',
          isSelected
            ? 'bg-[color-mix(in_srgb,var(--accent)_8%,transparent)] text-(--sidebar-text-active) ring-1 ring-[color-mix(in_srgb,var(--accent)_18%,transparent)]'
            : isTaskActive
              ? 'bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-(--sidebar-text-active) ring-1 ring-inset ring-[color-mix(in_srgb,var(--accent)_22%,transparent)]'
      ...
      >
        {task.worktreeBranch ? (
          <span title={task.worktreeBranch} className="relative flex shrink-0 items-center">
            <FolderGit2 className={cn('h-3.5 w-3.5', getWorktreeIconClass(task.workflowStatus))} />
            <ItemStatusIndicator
        ...
        >
          ...
          <div className="relative flex pointer-events-auto items-center gap-0.5">
            ...
            {!disableDnd && !isPending && (
              <SessionReferenceDragHint label={t('sidebar.dragSessionToReference')} />
            )}
            <ArchiveConfirmButton
...
export function ChatItemRow({
  ...
  return (
    <>
      ...
      <div
        draggable={!disableDnd && !isRenaming}
        onDragStart={!disableDnd && !isRenaming ? onDragStart : undefined}
        onDragEnd={!disableDnd && !isRenaming ? onDragEnd : undefined}
      ...
      >
        <span className="relative flex shrink-0 items-center">
          <MessageSquare className="h-3.5 w-3.5 text-(--text-muted) opacity-35" />
          <ItemStatusIndicator
            isProcessing={isProcessing}
            isAwaitingUser={isAwaitingUser}
            hasUnread={hasUnread}
