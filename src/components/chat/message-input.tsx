...
export function MessageInput({ sessionId, isDisabled, isReadOnly, isStopped, isSinglePanel = false }: MessageInputProps) {
  ...
  const canUseVoice = !isDisabled && !isGenerating;
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Single-line textarea + ArrowUp/Down → scroll message list
    // (multi-line: let default behavior handle line navigation)
    if ((e.key === 'ArrowUp' || e.key === 'ArrowDown') && !inputValue.includes('\n') && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
      ...
      <div className={cn('w-full', isSinglePanel ? SINGLE_PANEL_CONTENT_SHELL : 'px-4')}>
        ...
        {!isVoiceActive && (
          <button
            ...
            className={cn(
              'mb-2 shrink-0 rounded-md p-2 transition-all duration-150',
          ...
          <div className="flex-1 flex items-center min-h-[44px]">
            ...
            <textarea
              ...
              className={cn(
                ...
                !skillPicker.selectedSkill && 'px-4',
