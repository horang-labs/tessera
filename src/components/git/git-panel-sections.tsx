...
function FileDiffStats({ stats }: { stats: GitChangedFile["diffStats"] }) {
  ...
  return (
    <span
      ...
      title={`+${stats.added.toLocaleString()} -${stats.removed.toLocaleString()}`}
    >
      {stats.added > 0 ? (
...
export function GitPanelSummarySection({
  ...
  return (
    <div className="border-b border-(--chat-header-border) px-3 py-3">
  ...
  return (
    <div className="border-t border-(--chat-header-border) px-3 py-3 space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-1">
        ...
        <Tooltip content="Copy branch name" side="top">
          <Button
            variant="outline"
            size="icon"
        ...
        {data.repoUrl ? (
          <Tooltip content="Open repository" side="top">
            <Button
              variant="outline"
              size="icon"
        ...
        {data.prStatus?.url ? (
          <Tooltip content={`View PR #${data.prStatus.number}`} side="top">
            <Button
              variant="outline"
              size="icon"
              onClick={() => onOpenExternal(data.prStatus?.url)}
