...
import { SESSION_DRAG_MIME, TAB_DRAG_MIME } from '@/types/panel';
...
const tabItemVariants = cva(
  // base: always applied
  'electron-no-drag relative flex items-center select-none cursor-pointer' +
...
export const TabItem = memo(function TabItem({
  ...
  const projectDisplayName = useSessionStore(
    useCallback(
      (state) =>
        tab.projectDir
          ? state.projects.find((project) => project.encodedDir === tab.projectDir)?.displayName ?? tab.projectDir
          : null,
      [tab.projectDir],
    ),
  );
  const showProjectBadge = selectedProjectDir === ALL_PROJECTS_SENTINEL && tab.projectDir !== null;
  ...
  const handleDragStart = useCallback(
    function handleDragStart(e: React.DragEvent) {
      ...
      // Single-session tabs can be dropped onto panels (cross-tab DnD).
      const panels = isActive
        ? (selectActiveTab(usePanelStore.getState())?.panels ?? {})
        : (usePanelStore.getState().tabPanels[tab.id]?.panels ?? {});
      const sessionIds = Object.values(panels)
        .map((p) => p.sessionId)
        .filter(Boolean) as string[];
      if (sessionIds.length === 1) {
        e.dataTransfer.setData(SESSION_DRAG_MIME, sessionIds[0]);
      }
    },
