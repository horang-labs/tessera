...
import { setKanbanTaskDragData } from '@/lib/dnd/panel-session-drag';
...
export const KanbanTaskCard = memo(function KanbanTaskCard({
  ...
  const handleDragStart = useCallback((e: React.DragEvent) => {
    ...
    if (onDragStartProp) {
      ...
      setKanbanTaskDragData(e.dataTransfer, task.id, task.sessions[0]?.id);
      requestAnimationFrame(() => {
        useBoardStore.getState().setDragging(task.id);
      });
    }
  }, [isRenaming, onDragStartProp, task.id, task.sessions]);
  ...
  return (
    <>
      <div
        ...
        className={cn(
          'relative flex items-center gap-1.5 px-2 py-1 my-px rounded cursor-pointer',
